...
export default function FinancePage() {
  ...
  const handleRestore = async (id: string) => {
    ...
    setScripts(prev => prev.map(s => s.id === id ? { ...s, status: 'pending' } : s));
  };
    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-red-500 mb-2">
            <TrendingUp size={20} />
            <span className="text-xs font-bold uppercase tracking-widest">Finance Channel</span>
          </div>
          <h1 className="text-4xl font-bold tracking-tight">Finance Scripts</h1>
          <p className="text-zinc-500 mt-2">Review, approve, or reject finance scripts.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="vercel-card p-6">
          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1">Pending</p>
      ...
      {selectedScript && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4 md:p-8">
          <div className="bg-zinc-950 border border-white/10 w-full max-w-5xl max-h-[90vh] rounded-xl flex flex-col overflow-hidden">
            <div className="p-6 border-b border-white/10 flex items-center justify-between bg-zinc-900/50">
              <div>
                <h2 className="text-xl font-bold text-white">{selectedScript.title}</h2>
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-[10px] text-zinc-500 uppercase tracking-widest">{selectedScript.channel}</span>
                  <div className="w-1 h-1 rounded-full bg-zinc-700" />
                  <span className="text-[10px] text-zinc-500 uppercase tracking-widest">{selectedScript.duration}</span>
                </div>
              </div>
              <button onClick={() => setSelectedScript(null)} className="p-2 hover:bg-red-500/10 hover:text-red-500 rounded-full transition-all">
                <X size={24} />
              </button>
            </div>
                ...
                {selectedScript.content.split('\n').map((line, i) => {
                  ...
                  return (
                    <p key={i} className={cn(
                      ...
                      isHost && "text-white font-bold text-base mt-6 mb-2",
                      isHeading && "text-2xl font-black text-white border-b border-red-600/30 pb-4 mb-8 mt-10",
            ...
            <div className="p-6 border-t border-white/10 bg-zinc-900/80 flex flex-col md:flex-row justify-end gap-4">
              <button onClick={() => setSelectedScript(null)} className="px-8 py-3 rounded-md bg-white/5 text-white font-bold uppercase tracking-widest text-[10px] hover:bg-white/10 border border-white/10">Close</button>
              {selectedScript.status === 'pending' && (
                <>
                  <button onClick={() => { handleReject(selectedScript.id); setSelectedScript(null); }} className="px-8 py-3 rounded-md bg-red-600/10 text-red-500 font-bold uppercase tracking-widest text-[10px] hover:bg-red-600/20 border border-red-600/30">Reject</button>
                  <button onClick={() => { handleApprove(selectedScript.id); setSelectedScript(null); }} className="px-12 py-3 rounded-md bg-red-600 text-white font-bold uppercase tracking-widest text-[10px] hover:bg-red-700">Approve</button>
                </>
              )}
              {selectedScript.status === 'rejected' && (
                <button onClick={() => { handleRestore(selectedScript.id); setSelectedScript(null); }} className="px-12 py-3 rounded-md bg-blue-600 text-white font-bold uppercase tracking-widest text-[10px] hover:bg-blue-700">Restore to Pending</button>
              )}
            </div>
          </div>
        </div>
